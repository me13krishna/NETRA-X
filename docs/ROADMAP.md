...
Comprehensive multi-phase roadmap outlining the transition from functional MVP to competition-ready, production Cyber Threat Intelligence (CTI) & Attribution Operating System.
...
- **Definition of Done**: `pytest tests/test_backend.py` and `pytest tests/test_e2e_hero.py` pass 100% cleanly.

---

## Phase 1: Core Attribution Engine + Synthetic Excellence (Active)
- **Goals**: Refine mathematical Log-Likelihood Ratio (LLR) fusion, introduce dependence discounting ($\lambda=0.25$), family caps, isotonic calibration, and stylometric same-author verification.
- **Main Deliverables**:
...
- **Definition of Done**: Benchmark report outputs Expected Calibration Error (ECE) $< 0.15$, Brier Score $< 0.08$, False-Attribution Rate (FAR) $0.0\%$, and Recall@10 $100\%$.
...
- **Main Deliverables**:
  ...
  - `workers/extraction/extractor.py`: PGP key fingerprint, BTC, ETH, Monero (XMR hard abstention rule), handle, and email parsers.
  - `workers/extraction/clone_detector.py`: SimHash 64-bit structural clone detector ($\ge 95\% \to \text{phishing\_clone}$).
- **Definition of Done**: Passive crawlers write WARC artifacts to MinIO, trigger Redis stream, extract identifiers, and update PostgreSQL database idempotently.

---

## Phase 3: Premium Real-time Frontend (Active)
...
- **Definition of Done**: Analyst can complete the entire review workflow from queue $\to$ waterfall detail $\to$ review decision $\to$ export report seamlessly in the UI.

---

## Phase 4: Production Hardening & Observability (Upcoming)
- **Goals**: High-concurrency performance, zero-trust RBAC, OpenSearch full-text search indexing, and real-time activity stream.
- **Main Deliverables**:
...
- **Definition of Done**: System handles 10,000+ evidence items per minute with $< 200\text{ms}$ query latency and 100% audit log verification pass rate.
