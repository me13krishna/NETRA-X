...
- **Core Goal**: Make the system of record rock-solid, data pipelines reliable, and REST API complete.
...
- **Deliverables**:
  ...
  - `/api/v1/review/{hypothesis_id}`: Record analyst decisions and append SHA-256 audit log events.
...
3. Keep `tests/test_backend.py` and `tests/test_e2e_hero.py` passing.
